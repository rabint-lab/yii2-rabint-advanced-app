...
use app\modules\logistic\models\search\PriceLogSearch;
...
/**
 * AdminPriceLogController implements the CRUD actions for PriceLog model.
 */
class AdminPriceLogController extends \rabint\controllers\AdminController {
    ...
    public function actionBulk($action)
    {
        ...
        switch ($action) {
            case static::BULK_ACTION_SETPUBLISH:
                if (PriceLog::updateAll(['status' => PriceLog::STATUS_DRAFT], ['id' => $selection])) {
            ...
            case static::BULK_ACTION_DELETE:
                if (PriceLog::deleteAll(['id' => $selection])) {
                    Yii::$app->session->setFlash('success',  Yii::t('rabint', 'Bulk action was successful'));
                } else {
                    $err++;
                }
    ...
    /**
     * Lists all PriceLog models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new PriceLogSearch();
    ...
    /**
     * Displays a single PriceLog model.
     * @param integer $id
     * @return mixed
     */
            ...
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    return [
                        ...
                        'title' =>  Yii::t('rabint', 'Create new').' '. Yii::t('rabint', 'PriceLog'),
    ...
    /**
     * Updates an existing PriceLog model.
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    [
                        'forceReload' => '#ajaxCrudDatatable',
                        'title' =>  Yii::t('rabint', 'Updating').' '. Yii::t('rabint', 'PriceLog'),
    ...
    /**
     * Deletes an existing PriceLog model.
    ...
    /**
     * Finds the PriceLog model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PriceLog the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PriceLog::findOne($id)) !== null) {
            return $model;
        } else {
